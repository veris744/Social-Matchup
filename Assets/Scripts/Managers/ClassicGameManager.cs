...
public class ClassicGameManager : GameManager
{
    ...
    public void OnImageEnterGaze(int imageIndex, int playerId)
    {
        ...
        if (pvp)
        {
            ...
            {
                bool sameImageGazed1 = (playerId == 1 && images[2][imageIndex].GetComponent<Image>().IsGazed == true) ||
                                       (playerId == 2 && images[1][imageIndex].GetComponent<Image>().IsGazed == true);
                if(sameImageGazed1) destroyTeam1 = StartCoroutine(DestroyImage(imageIndex,1));
            }

            else
            {
                bool sameImageGazed2 = (playerId == 3 && images[4][imageIndex].GetComponent<Image>().IsGazed == true) ||
                                       (playerId == 4 && images[3][imageIndex].GetComponent<Image>().IsGazed == true);
            
            
                if(sameImageGazed2) destroyTeam2 = StartCoroutine(DestroyImage(imageIndex,2));
            }
    ...
    public void OnImageExitGaze(int imageIndex, int playerId)
    {
        if (pvp)
        {
            if (playerId == 1 || playerId == 2)
            {
                ...
                if(destroyTeam1!=null) StopCoroutine(destroyTeam1);
                images[1][imageIndex].GetPhotonView().RPC("StopDestroyAnimation", RpcTarget.All);
                images[2][imageIndex].GetPhotonView().RPC("StopDestroyAnimation", RpcTarget.All);
            }
            else
            {
                try
                ...
                if(destroyTeam2!=null) StopCoroutine(destroyTeam2);
                images[3][imageIndex].GetPhotonView().RPC("StopDestroyAnimation", RpcTarget.All);
                images[4][imageIndex].GetPhotonView().RPC("StopDestroyAnimation", RpcTarget.All);
            }
        }
        else
        {
